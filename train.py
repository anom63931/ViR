...
group.add_argument('--ampere_sparsity', action='store_true', default=False,
                    help='Save checkpoints')
group.add_argument('--activation_tracker', action='store_true', default=False,
                    help='track activations')
group.add_argument('--lmdb_dataset', action='store_true', default=False,
                    help='use lmdb dataset')
group.add_argument('--local_train', action='store_true', default=False,
                    help='use local training settings')
group.add_argument('--bfloat', action='store_true', default=False,
                    help='use bfloat datatype')
parser.add_argument('--fine_tune_21k', default='', type=str, metavar='PATH',
...
def main():
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.activation_tracker and (output_dir is not None):
                activation_catcher.attach_hooks(model)
                with torch.no_grad():
                    ...
                    activation_catcher.store_fmaps(folder=log_dir, filename='activation_stats.p',
                                                   epoch=epoch, plot=True)

                activation_catcher.detach_hooks()
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if (batch_idx == 0) or (batch_idx % 50 == 0):
            ...
            #     if hasattr(module, "set_ratio"):
            #         if args.warmup_epochs < epoch:
            #             module.set_ratio(ratio=lr / args.lr)
            #             if display_first:
            #                 print(f"Alpha ratio= {lr / args.lr}")
            #                 display_first = False
            #             # print(module, module.alpha, lr / args.lr)
